...
--SELECT  job_id,name from sysjobs where name like @nm+'%'
--'Onyx Log Backup - Recurring')
--select job_name= 'Onyx Log Backup - Recurring'

select 
job_name=b.name,
(C.active_end_time-C.active_start_time)/60/60 [duration],
schedule_name=C.name,
frequency_type=CASE C.freq_type
WHEN 1  THEN 'Once'
WHEN 4  THEN 'Daily'
WHEN 8  THEN 'Weekly'
WHEN 16 THEN 'Monthly'
WHEN 32 THEN 'Monthly relative'
WHEN 64 THEN 'when SQLServerAgent service starts' END,
interval=CASE
...
number_of_units=C.freq_subday_interval,
frequency_units=CASE c.freq_subday_type
